...
- [ ] Move React project files into `_react/` (src/, index.html, vite/tailwind/eslint/tsconfig configs)
- [ ] Scaffold SvelteKit 5 with `npx sv@latest create`
- [ ] Install core dependencies
- [ ] Configure `svelte.config.js` (adapter-static, fallback, base URL)
- [ ] Configure `vite.config.ts` (PWA manifest from `_react/vite.config.ts`, workbox, package-version)
...
- [ ] Copy API providers from `_react/src/api/` verbatim
- [ ] Copy database layer (fix react-hot-toast in migration.ts)
- [ ] Copy types, utils, config, services
- [ ] Write i18n conversion script + convert all 13 locales from `_react/src/i18n/`
...
- [ ] Document token map in `DESIGN-TOKENS.md`
