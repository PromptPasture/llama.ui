...
2. **Move** `src/` → `_react/src/`, `index.html` → `_react/index.html`, and all React config files (`vite.config.ts`, `postcss.config.js`, `tailwind.config.js`, `eslint.config.js`, `tsconfig*.json`) into `_react/` — keeps the React source as a side-by-side reference during porting. Delete `_react/` in Phase 7 before the PR.
...
6. Configure `vite.config.ts`: port the PWA manifest and workbox config verbatim from `_react/vite.config.ts`; add `vite-plugin-package-version`.
...
**Parallel:** Steps 3–6 can proceed in parallel with Phase 2 setup (branch + file moves must happen first).
...
1. **Copy from `_react/src/`** (no changes):
   - `api/` (all providers + `message-normalization.ts`, `sse-parser.ts`, `response-utils.ts`, `config-mapper.ts`)
   - `database/indexedDB.ts`
   - `database/localStorage.ts`
   - `database/migration.ts` — remove the `react-hot-toast` import; replace `toast.error(...)` with `console.error(...)` (toast will be rewired in Phase 5)
   - `types/` (all 6 type files)
   - `utils/` (all utility files)
   - `config/` (all config files + JSON)
   - `services/inference-service.ts`
2. **i18n conversion:** Write a Node script (`scripts/convert-i18n.mjs`) that reads each `_react/src/i18n/*.json` and writes it to `src/lib/i18n/` in svelte-i18n's flat format. Run and verify all 13 locales. Manually review keys with nested interpolation.
...
1. **`src/lib/state/app.svelte.ts`** — replaces `_react/src/store/app.tsx`:
...
2. **`src/lib/state/chat.svelte.ts`** — replaces `_react/src/store/chat.tsx`:
...
3. **`src/lib/state/inference.svelte.ts`** — replaces `_react/src/store/inference.tsx`:
   - `$state`: `provider`, `selectedModel`, `models`
   - Actions: `fetchModels`, `setProvider`
4. **`src/lib/state/modal.svelte.ts`** — replaces `_react/src/store/modal.tsx`:
...
7. **Delete `_react/`** — the React reference folder is no longer needed; remove it in a cleanup commit before opening the PR.
8. Update `MEMORY.md` with new stack facts.
9. Open PR: `feat/sveltekit` → `main`. Tag release after merge to trigger deploy.
